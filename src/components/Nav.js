...
// ----------------------------------------------------------
//  Logo renderer
//  If business.logo is set → show image
//  If empty             → show text with accent colour
// ----------------------------------------------------------
  ...
  if (business.logo) {
    return `
      <img
        ...
        style="
          height: ${business.logoHeight || 36}px;
          width: auto;
          display: block;
          object-fit: contain;
          ${extraStyle}
...
export function renderNav() {
  ...
  nav.innerHTML = `
    ...
    ">
      <a href="#hero" style="display:flex;align-items:center;text-decoration:none;flex-shrink:0;">
        ${logoHTML()}
      </a>
      ...
      <button id="menu-btn" aria-label="Open menu" style="
        display:none; background:none; border:none;
        cursor:pointer; padding:4px; color:#1A1A1A; flex-shrink:0;
      ">
        <svg id="icon-open" width="22" height="22" fill="none"
          stroke="currentColor" stroke-width="2" stroke-linecap="round">
          <line x1="3" y1="6"  x2="19" y2="6"/>
          <line x1="3" y1="11" x2="19" y2="11"/>
          <line x1="3" y1="16" x2="19" y2="16"/>
        </svg>
        <svg id="icon-close" width="22" height="22" fill="none"
          stroke="currentColor" stroke-width="2" stroke-linecap="round"
          style="display:none">
    ...
    ">
      ...
      ">${logoHTML('opacity:0.85;')}</div>

      ${['about','services','portfolio','testimonials','contact'].map(id => `
        <a href="#${id}" class="mobile-link" style="
      ...
      <a href="https://wa.me/${business.whatsapp}?text=${encodeURIComponent(business.whatsappMessage)}"
        target="_blank" style="
          ...
          margin:12px 20px 4px; padding:14px 24px; border-radius:99px;
          background:${brand.color}; color:#fff;
  ...
  const openMenu  = () => { menu.style.display='flex'; iconOpen.style.display='none';  iconClose.style.display='block'; btn.setAttribute('aria-label','Close menu'); };
  const closeMenu = () => { menu.style.display='none'; iconOpen.style.display='block'; iconClose.style.display='none';  btn.setAttribute('aria-label','Open menu');  };
